...
if state.status != "processing" && #global.camPos > 1
	var camX = global.camPos[0]
	var camY = global.camPos[1]
	var camZ = global.camPos[2]
	
	if exists(param.S)
		if {param.S >= -1 && param.S <= 2}
			T{param.S}
		else
			M117 "abort moveToCam, wrong Tool-Parameter P"
			M99
